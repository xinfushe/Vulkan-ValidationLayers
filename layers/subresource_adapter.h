...
struct SubresourceOffset : public Subresource {
    ...
    SubresourceOffset(const RangeOffsetEncoder& encoder, const VkImageSubresource& subres, const VkOffset3D& offset_);
...
class RangeOffsetEncoder : public RangeEncoder {
  public:
    // The default constructor for default iterators
    RangeOffsetEncoder()
    ...
    RangeOffsetEncoder(const VkImageSubresourceRange& full_range, const VkExtent3D& full_range_image_extent,
                       const AspectParameters* param);
    // Create the encoder suitable to the full range (aspect mask *must* be canonical)
    RangeOffsetEncoder(const VkImageSubresourceRange& full_range, const VkExtent3D& full_range_image_extent)
        : RangeOffsetEncoder(full_range, full_range_image_extent, AspectParameters::Get(full_range.aspectMask)) {}
    RangeOffsetEncoder(const RangeOffsetEncoder& from);
  ...
  protected:
    ...
    IndexType Decode1D(const IndexType& encode, SubresourceOffset& blit_decode) const;
    IndexType Decode2D(const IndexType& encode, SubresourceOffset& blit_decode) const;
  ...
  private:
    ...
    IndexType (RangeOffsetEncoder::*encode_offset_function_)(const SubresourceOffset&) const;
    IndexType (RangeOffsetEncoder::*decode_offset_function_)(const IndexType& encode, SubresourceOffset& blit_decode) const;
};
...
class SubresourceOffsetGenerator : public SubresourceOffset {
  public:
    ...
    SubresourceOffsetGenerator(const RangeOffsetEncoder& encoder, const VkImageSubresourceRange& range, const VkOffset3D& offset,
  ...
  private:
    const RangeOffsetEncoder* encoder_;
...
class RangeOffsetGenerator {
  public:
    RangeOffsetGenerator() : encoder_(nullptr), isr_pos_(), pos_(), aspect_base_() {}
    bool operator!=(const RangeOffsetGenerator& rhs) { return (pos_ != rhs.pos_) || (&encoder_ != &rhs.encoder_); }
    RangeOffsetGenerator(const RangeOffsetEncoder& encoder);
    RangeOffsetGenerator(const RangeOffsetEncoder& encoder, const VkImageSubresourceRange& subres_range, const VkOffset3D& offset,
                         const VkExtent3D& extent);
    inline const IndexRange& operator*() const { return pos_; }
    inline const IndexRange* operator->() const { return &pos_; }
    SubresourceOffsetGenerator& GetBlitSubresourceGenerator() { return isr_pos_; }
    SubresourceOffset& GetBlitSubresource() { return isr_pos_; }
    RangeOffsetGenerator& operator++();
