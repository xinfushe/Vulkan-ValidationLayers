...
SubresourceOffset::SubresourceOffset(const RangeOffsetEncoder& encoder, const VkImageSubresource& subres, const VkOffset3D& offset_)
...
RangeOffsetEncoder::RangeOffsetEncoder(const VkImageSubresourceRange& full_range, const VkExtent3D& full_range_image_extent,
...
void RangeOffsetEncoder::PopulateFunctionPointers() {
    // Select the encode/decode specialists
    if (limits_.offset.y == 1) {
        encode_offset_function_ = &RangeOffsetEncoder::Encode1D;
        decode_offset_function_ = &RangeOffsetEncoder::Decode1D;
    } else {
        encode_offset_function_ = &RangeOffsetEncoder::Encode2D;
        decode_offset_function_ = &RangeOffsetEncoder::Decode2D;
    }
}

IndexType RangeOffsetEncoder::Encode1D(const SubresourceOffset& pos) const { return pos.offset.x * OffsetXSize(); }

IndexType RangeOffsetEncoder::Encode2D(const SubresourceOffset& pos) const {
    return (pos.offset.x * OffsetXSize()) + (pos.offset.y * OffsetYSize());
}

IndexType RangeOffsetEncoder::Decode1D(const IndexType& encode, SubresourceOffset& blit_decode) const {
    blit_decode.offset.y = 1;
    blit_decode.offset.x = static_cast<int32_t>(encode / OffsetXSize());
    return (encode % OffsetXSize());
}

IndexType RangeOffsetEncoder::Decode2D(const IndexType& encode, SubresourceOffset& blit_decode) const {
...
static bool IsValid(const RangeOffsetEncoder& encoder, const VkImageSubresourceRange& bounds, const VkOffset2D& offset,
...
RangeOffsetGenerator::RangeOffsetGenerator(const RangeOffsetEncoder& encoder, const VkImageSubresourceRange& subres_range,
...
RangeOffsetGenerator& RangeOffsetGenerator::operator++() {
    mip_index_++;
    // NOTE: If all selected mip levels are done at once, mip_count_ is set to one, not the number of selected mip_levels
    if (mip_index_ >= mip_count_) {
        const auto last_aspect_index = aspect_index_;
