        ...
        if ignore_unassigned:
            ...
            for vuid in undef_set:
                if vuid.startswith('UNASSIGNED-'):
                    unassigned.add(vuid)
            undef_set = undef_set - unassigned
        # VUIDs of the type VUID-vkApiName-commandBuffer-recording are code-gen'd, don't show up in src but are covered
        cb_recording = set()
        for vuid in undef_set:
            if 'commandBuffer-recording' in vuid:
                cb_recording.add(vuid)
        undef_set = undef_set - cb_recording
        if (len(undef_set) > 0):
            ok = False
...
def main(argv):
    ...
    val_source.parse()
    # Ensure code-generated VUIDs are covered
    for vuid in val_json.implicit_vuids:
        if 'commandBuffer-recording' in vuid:
            val_source.implicit_vuids.add(vuid)
